...
class DatabaseConnector:
        ...
        with open(f"{spreadsheet_folder}/shipping_data_0.csv", "r") as spreadsheet_file_0:
            with open(f"{spreadsheet_folder}/shipping_data_1.csv", "r") as spreadsheet_file_1:
                with open(f"{spreadsheet_folder}/shipping_data_2.csv", "r") as spreadsheet_file_2:
                    ...
                    self.populate_first_shipping(csv_reader_0)
                    self.populate_second_shipping(
                        csv_reader_1, csv_reader_2)

    def populate_first_shipping(self, csv_reader_0):
        """
    ...
    def populate_second_shipping(self, csv_reader_1, csv_reader_2):
        """
    ...
    def insert_shipment(self, product_name, product_quantity, origin, destination):
        ...
        """
        self.cursor.execute(
            query, (product_id, product_quantity, origin, destination))
        self.connection.commit()
    ...
    def test_insertions(self):
        """
        Test if the data has been inserted by querying the database.
        """
        # Test querying the product table
        self.cursor.execute("SELECT * FROM product LIMIT 5;")
        products = self.cursor.fetchall()
        print("Products Table (First 5 Entries):")
        for product in products:
            print(product)

        # Test querying the shipment table
        self.cursor.execute("SELECT * FROM shipment LIMIT 5;")
        shipments = self.cursor.fetchall()
        print("\nShipments Table (First 5 Entries):")
...
if __name__ == '__main__':
    ...
    database_connector.test_insertions()
